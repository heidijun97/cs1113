...
set_insert: (eq a) => (v:a) -> set a -> set a
...
set_product: (s1: set a) -> (s2: set d) -> set (pair a d)
...
set_toString: (Serialize a,eq a) => set a -> String
...
-- pairelt takes an element h and a list b and returns list of all
--pairs, (h,e), where e is an element of b
pairelt: a-> list.list d-> list.list (pair a d)
...
pairlist:list.list a->list.list d ->list.list (pair a d)
pairlist nil lb= nil
...
set_toString (mkSet l1)= listString l1
